...
const AppointmentForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (user?.role === 'admin') {
          const doctorsResponse = await axios.get('https://emr-fb-1.onrender.com/api/auth/doctors');
          setDoctors(doctorsResponse.data);
        }
        
        // If in edit mode, fetch appointment data
        if (isEditMode) {
          const appointmentResponse = await axios.get(`https://emr-fb-1.onrender.com/api/appointments/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let response;
      if (isEditMode) {
        response = await axios.put(`https://emr-fb-1.onrender.com/api/appointments/${id}`, appointmentData, config);
      } else {
        response = await axios.post('https://emr-fb-1.onrender.com/api/appointments', appointmentData, config);
      }
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`https://emr-fb-1.onrender.com/api/appointments/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleComplete = async () => {
    try {
      await axios.patch(
        `https://emr-fb-1.onrender.com/api/appointments/${id}/complete`,
  ...
  const handleCancel = async () => {
    try {
      await axios.patch(
        `https://emr-fb-1.onrender.com/api/appointments/${id}/cancel`,
        { notes: formData.notes },
        {
          headers: {
            Authorization: `Bearer ${token}`,
